...
export default function EditVehiclePage() {
  ...
  function goBackToVehicles() {
    // Ако страницата е отворена през линк от системата (има history), правим normal back.
    // Ако е отворена директно (няма история), връщаме към листинга.
  ...
  useEffect(() => {
    (async () => {
      ...
      // 🔒 ако id липсва — не пускаме Supabase с undefined
      if (!id || typeof id !== "string") {
        setError("Грешка: липсва ID на автомобила.");
        setLoading(false);
        return;
      }
      ...
      const { data: v, error: e2 } = await supabase
        .from("vehicles")
        .select("id,customer_id,reg_number,vin,make,model,year,engine,mileage")
        .eq("id", id)
        .single();
      ...
      setRegNumber(veh.reg_number ?? "");
      setVin(veh.vin ?? "");
      setMake(veh.make ?? "");
      setModel(veh.model ?? "");
      setYear(veh.year ? String(veh.year) : "");
      setEngine(veh.engine ?? "");
  ...
  async function save(e: React.FormEvent) {
    ...
    if (!id || typeof id !== "string") return setError("Липсва ID на автомобила.");
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="text-4xl font-bold text-white tracking-tight">
            Редакция на автомобил <span className="text-orange-300">•</span>
          </h1>
        ...
        <div className="flex items-center gap-2">
          <Button type="button" variant="outline" className={accentOutline} onClick={goBackToVehicles}>
            ← Назад
          </Button>
        </div>
      </div>
                          ...
                          return (
                            <button
                              ...
                              className={[
                                ...
                                isActive ? "bg-orange-500/10 text-orange-100" : "text-white/90 hover:bg-white/5",
                              ].join(" ")}
                            >
                              <div className="flex items-center justify-between gap-3">
